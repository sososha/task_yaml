    ...
    if (task.details) {
        ...
        if (task.details.startDate || task.details.dueDate || task.details.completionDate) {
            ...
            if (task.details.startDate)
                result += `- Start: ${task.details.startDate}\n`;
            if (task.details.dueDate)
                result += `- Due: ${task.details.dueDate}\n`;
            if (task.details.completionDate)
                result += `- Completed: ${task.details.completionDate}\n`;
            result += '\n';
        }
...
export function registerTaskResources(server) {
    // タスク一覧リソース
    server.resource('tasks-list', 'tasks://list', async (uri, extra) => {
        // URLからクエリパラメータを取得
    ...
    // タスク詳細リソース
    server.resource('tasks-detail', 'tasks://detail/:taskId', async (uri, extra) => {
    ...
    // プロジェクト進捗レポートリソース
    server.resource('tasks-report', 'tasks://report', async (uri, extra) => {
        // URLからクエリパラメータを取得
        const params = new URLSearchParams(uri.search);
        const filePath = params.get('file') || 'tasks/main.yaml';
        try {
