...
import { loadFromFile, dumpAsYaml, dumpAsJson } from '../utils/enhanced-yaml.js';
...
export function resolvePath(filePath) {
    ...
    if (PROJECT_ROOT === '/' || PROJECT_ROOT === '') {
        ...
        if (DEBUG)
            console.log(`[DEBUG] PROJECT_ROOT is set to root directory, using current directory instead: ${effectiveProjectRoot}`);
    }
    // 実効的なタスクパスの計算
    ...
    if (TASK_PATH === '/tasks' || TASK_PATH.startsWith('/tasks/')) {
        ...
        if (DEBUG)
            console.log(`[DEBUG] TASK_PATH is incorrectly set to /tasks, using ${effectiveTaskPath} instead`);
    }
    // 空のパスチェック
    if (!filePath) {
        const defaultPath = path.join(effectiveTaskPath, 'main.yaml');
        if (DEBUG)
            console.log(`[DEBUG] Empty path, using default: ${defaultPath}`);
        return defaultPath;
    }
    // パスの正規化
    ...
    if (path.isAbsolute(filePath)) {
        if (DEBUG)
            console.log(`[DEBUG] Path is absolute: ${filePath}`);
        // 特別な処理: /tasks パスは常に許可
        if (filePath === '/tasks' || filePath.startsWith('/tasks/')) {
            ...
            if (DEBUG)
                console.log(`[DEBUG] Special handling for /tasks path: ${filePath} -> ${safePath}`);
            return safePath;
        }
        // セキュリティチェック: ルートディレクトリへのアクセス制限
        if (!ALLOW_ROOT_PATHS && !filePath.startsWith(effectiveProjectRoot)) {
            if (DEBUG)
                console.log(`[DEBUG] Access to root path not allowed, redirecting to PROJECT_ROOT`);
            // その他の制限付きパスはPROJECT_ROOT内に制限
            const safePath = path.join(effectiveProjectRoot, path.basename(filePath));
            if (DEBUG)
                console.log(`[DEBUG] Restricting to PROJECT_ROOT: ${filePath} -> ${safePath}`);
            return safePath;
        }
        // 制限がない場合は絶対パスをそのまま返す
        return filePath;
    ...
    if (filePath.startsWith('tasks/') || filePath === 'tasks') {
        ...
        if (DEBUG)
            console.log(`[DEBUG] tasks/ prefix resolved to: ${resolvedPath}`);
        return resolvedPath;
    }
    ...
    if (filePath.startsWith('./') || filePath === '.') {
        ...
        if (DEBUG)
            console.log(`[DEBUG] ./ prefix resolved to: ${resolvedPath}`);
        return resolvedPath;
    }
    ...
    if (DEBUG)
        console.log(`[DEBUG] Simple path resolved relative to PROJECT_ROOT: ${resolvedPath}`);
    return resolvedPath;
}
/**
 * タスクを作成する
 */
async function createTask(title, status = 'not_started', file, parentId, details) {
    try {
        if (!title)
        ...
        // タスクを作成
        const result = await createTaskWithPath(resolvedPath, title, status, parentId, taskDetails);
        return result;
    }
    catch (error) {
 ...
 */
async function createTaskWithPath(file, title, status, parentId, details) {
    try {
        if (!file) {
 ...
 * タスクを更新する
 */
async function updateTask(id, changes, file) {
    try {
        if (!id || !file) {
        ...
        // IDに一致するタスクを検索
        const taskIndex = existingData.tasks.findIndex((task) => task.id === id);
        if (taskIndex === -1) {
 ...
 * タスクを削除する
 */
async function deleteTask(id, file) {
    try {
        // パスを正規化
        const resolvedPath = resolvePath(file);
 ...
 * 初期テンプレートを作成する
 */
async function createInitialTemplate(projectName, projectDescription, basePath) {
    try {
        // 詳細なデバッグログ
