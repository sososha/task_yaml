...
// インクルード機能付きYAML読み込み
function loadWithIncludes(content: string, baseDir?: string): any {
  try {
...
export function formatTask(task: any): string {
  ...
  // ステータスアイコン
  const status = task.status as keyof typeof STATUS_ICONS;
...
export function dumpAsYaml(data: any, filePath: string, useFlowStyle: boolean = false): boolean {
  try {
...
// JSONとして保存
export function dumpAsJson(data: any, filePath: string, pretty: boolean = false): boolean {
  try {
...
export function dumpAsOneLine(data: any, filePath: string): boolean {
  try {
    // タスク配列の場合
    ...
    // オブジェクトの場合
      ...
      if (Array.isArray(clonedData.tasks)) {
        ...
        clonedData.tasks = clonedData.tasks.map((task: any) => {
          // オブジェクトでない場合はスキップ
